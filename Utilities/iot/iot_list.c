...
{
    list_t *self;
    if (!(self = iot_platform_malloc(sizeof(list_t)))) {
        return NULL;
    }
    self->head = NULL;
    self->tail = NULL;
...
void list_destroy(list_t *self)
{
    ...
    while (len--) {
        ...
        iot_platform_free(curr);
        curr = next;
    }

    iot_platform_free(self);
}
...
list_iterator_t *list_iterator_new_from_node(list_node_t *node, list_direction_t direction)
{
    ...
    if (!(self = iot_platform_malloc(sizeof(list_iterator_t)))) {
        return NULL;
    }
    self->next = node;
...
{
    list_node_t *self;
    if (!(self = iot_platform_malloc(sizeof(list_node_t)))) {
        return NULL;
    }
