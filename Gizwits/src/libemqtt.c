...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
	...
	uint16_t payload_len = clientidlen + 2;
       ...
       uint8_t    mqtt_connect_send_buf[BUF_LEN];
	...
	memcpy(packet, fixed_header, sizeof(fixed_header));
       TRACE_INFO("ripple---->, packet size = %d, fixheader[0]=%x, fixheader[1] =%x", sizeof(packet), fixed_header[0], fixed_header[1]);
	...
	memcpy(packet+offset, broker->clientid, clientidlen);
       TRACE_INFO("ripple---->, broker->clientid=%s, len=%d", broker->clientid, clientidlen);
	offset += clientidlen;
	...
	if(passwordlen) {
        ...
        HexToStr((unsigned char *)debug_info, (unsigned char *)packet, sizeof(packet));
        TRACE_INFO( "mqtt connect debug, len: %d, bytes: %s", sizeof(packet), debug_info);


        memset(mqtt_connect_send_buf, 0, sizeof(mqtt_connect_send_buf));
        memcpy(mqtt_connect_send_buf, packet, sizeof(packet));
	// Send the packet
	//TRACE_INFO("ripple----> before mqtt connect, packet=%s", packet);
	if(broker->send(broker->socket_info, mqtt_connect_send_buf, sizeof(packet)) < (sizeof(packet))) {
		return -1;
	}
...
int mqtt_publish(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint8_t retain) {
	return mqtt_publish_with_qos(broker, topic, msg, strlen(msg), retain, 0, NULL);
}
