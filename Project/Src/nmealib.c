...
bool attr_GGA_handler_loc(u8 *attr, unsigned int len, u8 index, u8 **data){
  ...
  memset(tmp, 0x00, 4);
  if(index == 2){
    if((len != 9 && len != 7 && len != 10) || attr[4] != '.') // format is "ddmm.mmmm" or "ddmm.mm" or "ddmm.mmmmm"
      return true;    
    tmp[0] = attr[0];
    tmp[1] = attr[1];
    ...
    if(len == 7){
      ...
      tmp[0] = attr[9];
      tmp[1] = 0x0;
      val->lat.minp3 = strtol(tmp, NULL, 10);         
    }
  }
  else if(index == 4){
    ...
    if((len == 9 || len == 7 || len == 10) && attr[4] == '.'){  // format is "ddmm.mmmm" or "ddmm.mm" or "ddmm.mmmmm"
      ...
      if(len == 7){
        ...
        tmp[0] = attr[9];
        tmp[1] = 0x0;
        val->lon.minp3 = strtol(tmp, NULL, 10);         
      }
    }
    else if((len == 10 || len == 8 || len == 11) && attr[5] == '.') {// format is "dddmm.mmmm" or "dddmm.mm" or "ddmm.mmmmm"   
      tmp[0] = attr[0];
      tmp[1] = attr[1];
      tmp[2] = attr[2];
      val->lon.deg = strtol(tmp, NULL, 10);
...
/**
 * Parse Horizontal Dilution of precision ( HDOP 0.5 - 99.9 meters)
 *
 * @param attr the string of time info
 * @param len the length of the string
 * @param index info the attr index of this line
 ...
 * @return true on success, false otherwise
 */
bool attr_GGA_handler_HDOP(u8 *attr, unsigned int len, u8 index, u8 **data){
  char tmp[3];
  data_store_t *val = (data_store_t *) *data;
  memset(tmp, 0x00, 3);  

  if((len == 4) && attr[1] == '.'){   /* x.xx */  
    tmp[0] = attr[0];
    val->hdop.integ = strtol(tmp, NULL, 10);
    tmp[0] = attr[2];
    tmp[1] = attr[3];
    val->hdop.dec = strtol(tmp, NULL, 10);
  }
  else if((len == 5) && attr[2] == '.'){    /* xx.xx */
    tmp[0] = attr[0];
    tmp[1] = attr[1];
    val->hdop.integ = strtol(tmp, NULL, 10);
    tmp[0] = attr[3];
...
/**
 * Antenna Altitude above/below mean-sea-level (geoid) (in -9999.9 - 99999.9 meters)
 *
 * @param attr the string of time info
 * @param len the length of the string
 * @param index info the attr index of this line
 ...
 * @return true on success, false otherwise
 */
bool attr_GGA_handler_AMSL(u8 *attr, unsigned int len, u8 index, u8 **data){
  char tmp[6];
  data_store_t *val = (data_store_t *) *data;
  u8 integer_len = 0;
  memset(tmp, 0x00, 6);
  
  for(integer_len = 1; integer_len < 6; integer_len ++){
    if(integer_len >= len - 1)
      return true;
    if(attr[integer_len] == '.' )
      break;
  }
  if(integer_len >= 6)
    return true;
  memcpy(tmp, attr, integer_len);
  val->amsl.integ = strtol(tmp, NULL, 10);
  
  tmp[0] = attr[integer_len + 1];
  tmp[1] = 0x0;
  val->amsl.dec = strtol(tmp, NULL, 10);
  return true;
}
...
bool attr_GGA_handler_header(u8 *attr, unsigned int len, u8 index, u8 **data){
  u8 header[] = "$GPGGA";
  u8 header1[] = "$GNGGA";
  if(0 != memcmp(attr, header, strlen((char const *)header)) && 0 != memcmp(attr, header1, strlen((char const *)header1)))
    return false;
#if ! defined(RESOLVED_DATA_TYPE)
...
* 8. Horizontal Dilution of precision ( HDOP 0.5 - 99.9 meters)
* 9. Antenna Altitude above/below mean-sea-level (geoid) (in -9999.9 - 99999.9 meters)
...
attr_GGA_handler attr_GGA_handler_list[]={
  ...
  attr_GGA_handler_HDOP,      //8 precision of hori 
  attr_GGA_handler_AMSL,      //9
